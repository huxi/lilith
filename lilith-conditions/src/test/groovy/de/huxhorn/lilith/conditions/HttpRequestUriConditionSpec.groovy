...
class HttpRequestUriConditionSpec extends Specification {
	@Unroll
	def "Corpus works as expected for #condition (searchString=#input)."() {
		...
		condition = new HttpRequestUriCondition(input)
	}

	@Unroll
	def "serialization works with searchString #input."() {
		when:
		def condition = new HttpRequestUriCondition()
	...
	def "equals behaves as expected."() {
		setup:
		def instance = new HttpRequestUriCondition()
		def other = new HttpRequestUriCondition('foo')
