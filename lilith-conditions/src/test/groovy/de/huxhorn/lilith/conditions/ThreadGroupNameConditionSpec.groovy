...
public class ThreadGroupNameConditionSpec extends Specification
{
	@Unroll
	def "Corpus works as expected for #condition (searchString=#input)."() {
		...
		condition = new ThreadGroupNameCondition(input)
	}
	...
	def "serialization works with searchString #input."() {
		when:
		def condition = new ThreadGroupNameCondition()
	...
	def "equals behaves as expected."() {
		setup:
		def instance = new ThreadGroupNameCondition()
		def other = new ThreadGroupNameCondition('foo')
