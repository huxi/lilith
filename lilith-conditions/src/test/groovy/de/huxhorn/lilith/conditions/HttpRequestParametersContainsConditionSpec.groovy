...
class HttpRequestParametersContainsConditionSpec extends Specification {
	...
	def "Corpus works as expected for #condition (key=#key, value=#value)."() {
		...
		null                  | null                     | [] as Set
		'requestParameterKey' | null                     | [110, 111, 112] as Set
		'requestParameterKey' | 'requestParameterValue3' | [112] as Set

		condition = new HttpRequestParametersContainsCondition(key, value)
	}

	@Unroll
	def "serialization works with key=#key and value=#value."() {
		when:
		def condition = new HttpRequestParametersContainsCondition()
		condition.key = key
	...
	def "equals behaves as expected."() {
		setup:
		def instance = new HttpRequestParametersContainsCondition()
		def other = new HttpRequestParametersContainsCondition(key, value)
