...
class HttpMethodConditionSpec extends Specification {
	@Unroll
	def "Corpus works as expected for #condition (searchString=#input)."() {
		...
		condition = new HttpMethodCondition(input)
	}

	@Unroll
	def "serialization works with searchString #input."() {
		when:
		def condition = new HttpMethodCondition()
	...
	def "equals behaves as expected."() {
		setup:
		def instance = new HttpMethodCondition()
		def other = new HttpMethodCondition('foo')
