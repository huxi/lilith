...
class MessagePatternEqualsConditionSpec extends Specification {
	@Unroll
	def "Corpus works as expected for #condition (searchString=#input)."() {
		...
		condition = new MessagePatternEqualsCondition(input)
	}
	...
	def "equals behaves as expected."() {
		setup:
		def instance = new MessagePatternEqualsCondition()
		def other = new MessagePatternEqualsCondition('foo')
