...
import de.huxhorn.lilith.log4j2.decoder.Log4j2JsonDecoder;
import de.huxhorn.sulky.buffers.AppendOperation;
import java.io.IOException;
import java.io.InputStream;

@SuppressWarnings("PMD.ClassNamingConventions")
public class Log4j2JsonServerSocketEventSourceProducer
		extends AbstractServerSocketEventSourceProducer<LoggingEvent>
{
	public Log4j2JsonServerSocketEventSourceProducer(int port)
			throws IOException
	{
		super(port);
	}
	...
	protected EventProducer<LoggingEvent> createProducer(SourceIdentifier id, AppendOperation<EventWrapper<LoggingEvent>> eventQueue, InputStream inputStream)
			throws IOException
	{
		return new ZeroDelimitedEventProducer<>(id, eventQueue, new LoggingEventSourceIdentifierUpdater(), new Log4j2JsonDecoder(), inputStream);
	}

	@Override
	public String toString()
	{
		return "Log4j2JsonServerSocketEventSourceProducer[port=" + getPort() + "]";
	}
